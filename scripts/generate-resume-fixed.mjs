...
function sectionTitle(title) {
  doc.moveDown(0.8);
  doc.font("Helvetica-Bold").fontSize(12).fillColor(colors.text).text(title.toUpperCase());
  const y = doc.y + 4;
  doc
...
function linkLine(label, url) {
  doc.font("Helvetica").fontSize(10.5).fillColor(colors.text).text(`${label}: `, { continued: true });
  doc.fillColor(colors.accent).text(url, { link: url, underline: true });
  doc.fillColor(colors.text);
}

doc.font("Helvetica-Bold").fontSize(24).fillColor(colors.text).text("Laxmi Godara", { align: "center" });
doc.moveDown(0.2).font("Helvetica").fontSize(11).fillColor(colors.muted).text("Full-Stack Developer | MERN Stack", { align: "center" });
doc.moveDown(0.4).font("Helvetica").fontSize(10.5).fillColor(colors.text).text("lgodara285@gmail.com | 7976533737", { align: "center" });

doc.moveDown(0.2);
doc.fillColor(colors.accent).text("LinkedIn", {
  align: "center",
  continued: true,
  link: "https://www.linkedin.com/in/laxmi-godara-b1b0b6241",
  underline: true,
});
doc.fillColor(colors.muted).text("  |  ", { align: "center", continued: true });
doc.fillColor(colors.accent).text("GitHub", {
  align: "center",
  continued: true,
  link: "https://github.com/LaxmiGodara",
  underline: true,
});
doc.fillColor(colors.muted).text("  |  ", { align: "center", continued: true });
doc.fillColor(colors.accent).text("Portfolio", {
  align: "center",
  continued: true,
  link: "https://laxmigodara.netlify.app",
  underline: true,
});
doc.fillColor(colors.muted).text("  |  ", { align: "center", continued: true });
doc.fillColor(colors.accent).text("Twitter", {
  align: "center",
  link: "https://x.com/LaxmiCodes",
  underline: true,
});

sectionTitle("Summary");
doc.font("Helvetica").fontSize(10.8).fillColor(colors.text).text(
